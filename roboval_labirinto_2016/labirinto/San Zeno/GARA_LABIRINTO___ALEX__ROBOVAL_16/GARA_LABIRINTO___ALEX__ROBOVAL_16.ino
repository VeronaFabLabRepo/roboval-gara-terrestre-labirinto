...
//    #include <QTRSensors.h> 
    ...
    #define full_speed          40 //velocita massima
    #define outline_speed    45    //velocita quando il robot non è perfettamente sulla strada (=velocita massima -> non serve a niente :) )
    #define turn_speed       25    //velocita in manovra (rotazione a destra , sinistra)
    #define blind_speed          22 //velocita motori in inversione di marcia
    #define debugPeriod       1000  //costanti Roboval non modificate
    #define soglia             110 //soglia tra 0 e 1000 per commutare il valore alto/basso analogico proveniente dai sensori in digitale 
...
#define BORNOUT { motor_brake(1); motor_brake(0); delay(500); }  
 ...
 //esso deve semplicemente vedere se uno dei valori SUMsx O SUMdx (DERIVATI DAI SENSORI) SONO COMPRESI NEL MARGINE DI +-  UN TOT PER CENTO DI ESSI STESSI (X ESEMPIO SE SUMdx è COPRESO FRA SUMSX + O -  IL SUO 5 PER CENTO ) 
    ...
    float hardware_deceleration = 0.1;
...
void straight(){
 ...
 void turnLeft(){
   ...
   set_motor(left_motor, (full_speed-full_speed* hardware_deceleration/100) + speedAdder);
   set_motor(right_motor, full_speed);
   //delay(10);
   //BORNOUT;
   //ECCO CHE FORZO I MOTORI A GIRARE A PRIORI
    set_motor(left_motor,-turn_speed+turn_speed*hardware_deceleration/100 + speedAdder);
 ...
 void turnRight(){
   ...
   set_motor(right_motor, full_speed + speedAdder);
  delay(10);
  readSensors();
  ...
  if(!(rightNearReading || leftNearReading)){
    ...
    set_motor(right_motor,-turn_speed + speedAdder);
