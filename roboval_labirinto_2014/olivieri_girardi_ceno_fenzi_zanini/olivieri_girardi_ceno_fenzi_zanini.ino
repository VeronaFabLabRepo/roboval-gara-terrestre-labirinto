...
#define correction_speed    38 // velocita' di correzione traiettoria rettilinea (0-100)
...
void straight(){
  if(!leftNearReading){// 0x01x0 il robot e' troppo a sinistra, deve andare piu' a destra quindi alzo il motore sinistro
    set_motor(left_motor, full_speed/2);
    set_motor(right_motor, correction_speed/3);
    return;
  }
  else if(!rightNearReading){// 0x10x0 il robot e' troppo a destra, deve andare piu' a sinistra quindi alzo il motore destro
    set_motor(left_motor, correction_speed/3);
    set_motor(right_motor, full_speed/2);
    return;
  }
...
void turnAround(){
  delay(attesaPerManovra);
  while(!leftNearReading || !rightNearReading){
    set_motor(left_motor,-turn_speed/2);
    set_motor(right_motor,turn_speed/2);
    readSensors();
  }
}
void readSensors(){
