...
#define debugPeriod       50 // ogni quanti millisecondi inviare i messaggi di debug
#define soglia             320 // soglia di riconoscimento bianco/nero (0-1000)
#define attesaPerManovra    20 // tempo di attesa tra il riconoscimento della curva e il comando ai motori
#define correction_two     4
#define around_speed       35
...
void loop(){
  ...
  if (leftCenterReading && rightCenterReading && leftNearReading){
    straightRight();
  }
    else if (leftCenterReading && rightCenterReading && rightNearReading){
    straightLeft();
  }

  else if(!leftFarReading && !rightFarReading && (leftNearReading || rightNearReading) ){  // 0x1xx0 || 0xx1x0
    straight();
  }
 else if (!leftFarReading && !leftNearReading && leftCenterReading && rightCenterReading && leftNearReading && leftFarReading) {
  turnRight();
  
}
 else if (rightFarReading && rightNearReading && rightCenterReading && leftCenterReading && leftNearReading && leftFarReading){
  OLeft();
}
...
void straight(){
  if(rightNearReading && !leftCenterReading){// 0x01x0 il robot e' troppo a sinistra, deve andare piu' a destra quindi alzo il motore sinistro
    
    //set_motor(left_motor, correction_two);
    set_motor(right_motor, correction_speed);
    //set_motor(right_motor, full_speed);
    set_motor(left_motor, full_speed);
    return;
  }
  else if(leftNearReading && !rightCenterReading){// 0x10x0 il robot e' troppo a destra, deve andare piu' a sinistra quindi alzo il motore destro
    set_motor(left_motor, correction_speed);
    //set_motor(right_motor, correction_two);
    set_motor(right_motor, full_speed);
    //set_motor(left_motor, full_speed);
    return;
  }
...
void leftHandWall(){
  ...
  if(leftFarReading && !rightFarReading){ // 1xxxxx
    turnLeft();
  ...
  if (leftFarReading && leftNearReading && (leftCenterReading || !leftCenterReading)&& rightCenterReading && !rightNearReading){
  turnLeft();
}
...
void turnAround(){
  delay(attesaPerManovra);
  while(!leftNearReading || !rightNearReading){
    set_motor(left_motor,-around_speed);
    set_motor(right_motor,around_speed);
    readSensors();
  }
}
