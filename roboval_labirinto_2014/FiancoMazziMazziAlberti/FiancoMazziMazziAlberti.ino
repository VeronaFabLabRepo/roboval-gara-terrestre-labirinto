...
#define correction_speed    20  // velocita' di correzione traiettoria rettilinea (dei due center) (0-100)
...
#define sogliaesterna        0 // soglia dei due sensori esterni
#define correctionspeed     15 // velocità che deve avere quando i sensori center e near superano la soglia
#define correctionspeed2    10 // velocità che deve avere quando i due sensori near e far superano la soglia
...
void loop(){
  ...
  if(!leftFarReading && !rightFarReading && (leftNearReading || leftCenterReading || rightCenterReading || rightNearReading) ){  // 0x1xx0 || 0xx1x0
    straight();
  }
  else{
    leftHandWall(); //gira
...
void straight(){
 ...
 //   set_motor(right_motor, correction_speed);
  //  return;
 // }
//  else if(leftCenterReading){// 0x01x0 il robot e' troppo a sinistra, deve andare piu' a destra quindi alzo il motore sinistro
  ...
  if(!leftNearReading){// 0x01x0 il robot e' troppo a sinistra, deve andare piu' a destra quindi alzo il motore sinistro
    set_motor(left_motor, full_speed);
    set_motor(right_motor, correctionspeed);
    return;
  }
  else if(!rightNearReading){// 0x10x0 il robot e' troppo a destra, deve andare piu' a sinistra quindi alzo il motore destro
    set_motor(left_motor, correctionspeed);
    set_motor(right_motor, full_speed);
    return;
  }
    else if(leftCenterReading && leftNearReading){// quando il centrale sinistro e il near sinistro sono coperti mi utilizza una correction speed minore in modo da essere più rapido nel mettersi dritto
    set_motor(left_motor,  correctionspeed);
    set_motor(right_motor, full_speed);
    return;
  }  
   // else if(leftNearReading && leftFarReading){// quando il near e il far sinistri(i più distanti) sono soperti la correction speed è minima in modo da essere ancora più rapidi nel mettersi dritti
  //  set_motor(left_motor,  correctionspeed2);
   // set_motor(right_motor, full_speed);
   // return;
 // }  
 //   else if(rightNearReading && rightFarReading){// quando il near e il far destri(i più distanti) sono soperti la correction speed è minima in modo da essere ancora più rapidi nel mettersi dritti
   // set_motor(right_motor,  correctionspeed2);
   // set_motor(left_motor, full_speed);
  //  return;
 // }  
    else if(rightCenterReading && rightNearReading){// quando il centrale destro e il near destro sono coperti mi utilizza una correction speed minore in modo da essere più rapido nel mettersi dritto
    set_motor(right_motor,  correctionspeed);
    set_motor(left_motor, full_speed);
    return;
  }  
  //else if(!leftFarReading && !leftNearReading && rightFarReading && rightNearReading){// quando tutti e tre i right sono coperti gira a destra
  //  set_motor(right_motor, -full_speed);
  //  set_motor(left_motor, full_speed);
  //  return;
  //}  
  //else if(leftFarReading && leftNearReading){// quando tutti e tre i left sono coperti gira a sinistra
 //   set_motor(right_motor, full_speed);
  //  set_motor(left_motor, -full_speed);
   // return;
 // }  
...
void leftHandWall(){
  ...
  if(!leftFarReading && rightFarReading){  // 0xxxx1  // se il sinistro è vuoto gira a destra
    turnRight();
  }
  ...
  if(!rightFarReading && leftFarReading && leftNearReading){
    set_motor(left_motor,  correctionspeed2);
    set_motor(right_motor, full_speed);
  }
  if(!leftFarReading && rightFarReading && rightNearReading){
    set_motor(right_motor,  correctionspeed2);
    set_motor(left_motor, full_speed);
  }
}
...
void turnRight(){
  delay(attesaPerManovra);
  while(!leftNearReading || !rightNearReading){     //finche i due near non sono vuoti continua a girare
    set_motor(left_motor,turn_speed);
    set_motor(right_motor,-turn_speed);         
    readSensors();
  }
