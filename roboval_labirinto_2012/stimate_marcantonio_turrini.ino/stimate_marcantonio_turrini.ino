 /*  
   ...
   */ 
    ...
    #define full_speed          37 //velocita massima
    #define outline_speed    37    //velocita quando il robot non è perfettamente sulla strada (=velocita massima -> non serve a niente :) )
    #define turn_speed       23    //velocita in manovra (rotazione a destra , sinistra)
    #define blind_speed          25 //velocita motori in inversione di marcia
...
void straight(){
 ...
 void turnRight(){
  ...
  if(!(rightNearReading || leftNearReading)){
    set_motor(left_motor,turn_speed+5-turn_speed*hardware_deceleration/100);
...
//ASI UNGUAL ALLA VOSTRA SOLO CON QUALCHE PICCOLA MODIFICA : L'HARDWARE DEC....
  ...
  while(!leftNearReading && !rightNearReading){
    ...
    set_motor(left_motor,-blind_speed+blind_speed*hardware_deceleration/100);
    set_motor(right_motor,+blind_speed);
    readSensors();
