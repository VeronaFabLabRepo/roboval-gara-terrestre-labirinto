 ...
 *@autori: Alessandra Girardi, Michele Romani, Lorenzo Lioce
 ...
 * Contest Roboval 2017
 * I concorrenti sono liberi di modificare a piacimento questo firmware per migliorare il comportamento del robot
 * per infromazioni www.roboval.it
 *
...
#define turn_speed          25// velocita' di curvatura (0-100)


#define debugPeriod        300// ogni quanti millisecondi inviare i messaggi di debug
...
void loop(){
  ...
  if(!leftFarReading && !rightFarReading && (leftNearReading ||/*=or*/ rightNearReading) ){  // 0x1xx0 || 0xx1x0
...
void rightHandWall(){
  ...
  if(rightFarReading){ // 1xxxxx

    turnRight();

  }

  if(!rightFarReading && leftFarReading){  // 0xxxx1
    turnLeft();

  }

  if(!rightFarReading && !rightCenterReading && !rightNearReading

    && !leftFarReading && !leftCenterReading && !leftNearReading){ // 000000
