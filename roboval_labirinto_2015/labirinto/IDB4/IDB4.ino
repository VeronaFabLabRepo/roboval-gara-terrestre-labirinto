...
#define correction_speed    15 // velocita' di correzione traiettoria rettilinea (0-100)
...
#define correction_speed_media 13
#define correction_speed_bassa 10
...
void loop(){
  ...
  else if (leftFarReading && leftNearReading /*&& leftCenterReading && rightCenterReading*/ && rightNearReading && rightFarReading){
    turnLeft();
  }
  else{
    leftHandWall(); //gira
...
void straight(){
  ...
  else if(leftCenterReading && !rightNearReading){       
    set_motor(right_motor, full_speed);
    set_motor(left_motor, correction_speed_media);
    return;
  }
  else if(rightCenterReading && !leftNearReading){
    set_motor(left_motor, full_speed);
    set_motor(right_motor, correction_speed_media);
    return;
  }/*  
  else if(rightFarReading && rightCenterReading){
    set_motor(left_motor, correction_speed_bassa);
    set_motor(right_motor, full_speed);
    return;
  }
  else if(leftFarReading && leftCenterReading){
    set_motor(left_motor, full_speed);
    set_motor(right_motor, correction_speed_bassa);
    return;
  }  */
...
void leftHandWall(){
  ...
  else if(!leftFarReading && rightFarReading){  // 0xxxx1
    turnRight();
    return;
  }/*
  else if(leftFarReading && leftNearReading){    //11xxxx
    turnLeft();
    return;
  }
  else if (!leftFarReading && rightFarReading && rightNearReading){      //0xxx11
    turnRight();
    return;
  }
  else if(!leftFarReading && !leftCenterReading && !leftNearReading && !rightFarReading && !rightCenterReading && !rightNearReading){ // 000000
    turnAround();
    return;
  }*/
  else{
