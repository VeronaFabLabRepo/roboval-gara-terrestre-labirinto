...
void leftHandWall(){
  if((!leftFarReading && rightFarReading) || (leftCenterReading && rightFarReading && !leftFarReading)){  // 0xxxx1  01xxx1 // se il sinistro è vuoto gira a destra
    dx=1;
    turnRight();
  }
  if(leftFarReading && !rightFarReading) { // 1xxxx0
    turnLeft();
    dx=0;
  }
  if (((leftFarReading && rightFarReading) || ((rightCenterReading && leftFarReading) && !rightFarReading))){ //1xxxx1  1xxx1x  
    dx=0;
    turnLeft();
  }  
  if ((!leftFarReading && !leftCenterReading && !leftNearReading && !rightFarReading && !rightCenterReading && !rightNearReading)){ //000000    
    dx=0;
    turnLeft();
  } 
  ...
  if((!leftFarReading && rightFarReading) && rightNearReading){// xxxx11
    turnRight();
  }
}
